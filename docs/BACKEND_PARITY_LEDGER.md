...
- `BE-032`: in progress; IMAP runtime defaults now point at Gluon, backend-selection settings have been removed from the persisted/runtime config surface, old JSON keys are ignored on load, and CI exercises the Gluon parity suites. Remaining work is the final release-candidate checklist and operator-facing cutover docs.
