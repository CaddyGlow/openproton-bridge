...
- `BE-030`: in progress with IMAP, IDLE, event-worker parity coverage, explicit compat-helper naming, and a live mail-runtime IMAP probe on the Gluon backend that now covers startup, offline `LOGIN`, authenticated `LIST`/`SELECT`/`FETCH`/`SEARCH`, authenticated `COPY`/`MOVE`/`STORE`/`EXPUNGE` upstream sync, direct connector-driven `IDLE` flag updates, event-worker-driven `IDLE` message-create updates, and event-worker-driven delete updates surfaced through `NOOP`
