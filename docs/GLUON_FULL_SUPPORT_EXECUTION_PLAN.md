...
- `BE-032` is in progress. CI coverage exists for the Gluon backend, the runtime defaults IMAP read and mutation backends to Gluon, and production startup now ignores persisted compat backend selections. Remaining work is the release-candidate acceptance checklist plus operator-facing cutover and rollback docs.
...
- Gluon is the default runtime backend, and persisted compat backend selections are ignored on the production startup path
...
Production startup no longer honors compat backend selection. Keep legacy compat parsing only for backward-compatible settings payloads and internal coverage until all are true:
