...
- `BE-030` is in progress. IMAP read/mutation/IDLE parity and multiple event-worker Gluon paths are covered, and there is now a live mail-runtime IMAP probe on the Gluon defaults that exercises startup, offline `LOGIN`, and authenticated `SELECT`, but broader authenticated runtime command parity and final helper cleanup are not yet fully closed.
