...
- A manual private-fixture parity harness now exists in `tests/gluon_real_fixture.rs`. It accepts either `OPENPROTON_REAL_GLUON_PROFILE=/path/to/bridge-v3` or `OPENPROTON_REAL_GLUON_ARCHIVE=/path/to/profile.tar` and proves `CompatibleStore::open_read_only` can enumerate upstream mailboxes and snapshots from a real local cache without checking private fixture data into the repo.
